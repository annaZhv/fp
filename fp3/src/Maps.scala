object Maps extends App {
  ...
  def testGroupUsers(users: Seq[User]): Map[String, Int] = {
    ...
    for (e <- a) {
      var cc= e._2.toBuffer.foldLeft[Int](0)((y, next) => y + next.age)/e._2.toBuffer.size
      map+=(e._1->cc)
    }
    map
  }
   ...
   *    Вы можете реализовать ваше решение в теле тестовой функции. Не изменяйте сигнатуру.
   */
  def testNumberFrodos(map: Map[String, User]): Int = (for (elem <- map if elem._2.name.contains("Adam")) yield elem).size;
